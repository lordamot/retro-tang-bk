...
// the reset, the AZ's cold reset, a "Hardware" form for the switches,
// an "About" text, the "Debug" window and "Save settings".  Every
// switch is a letter sysctrl.v decodes; the unit slots are sdc.c's
// image names, mounted through azbk.c (az_set_unit) and never through
// sd_card.v.
//
...
static const char *about_bk[] = {
  "BK Nano " CORE_VERSION " - a BK-0011M with an AZBK controller on a Tang Nano 20K",
  "",
...
static const char *settings_file[] = {
  ...
  CARD_MOUNTPOINT "/bk/bk.ini"     // core id = 10 CORE_ID_BK
};
...
// BK Nano: an image chosen in the OSD becomes the AZ's unit, and is
// remembered by name like any slot's; az_boot() sets the units from
// AZ.INI first, then these override the ones that were chosen here
...
#endif
{
  ...
  for(int drive=0;drive<MAX_DRIVES;drive++) {
    ...
    if(name) {
      ...
      if(core_id == CORE_ID_BK) menu_bk_mount(drive, local_name);
      else sdc_image_open(drive, local_name);
    }
  }
  } else
  ...
  // cold reset the core and hold it; for BK Nano the AZ's ROMs and
  // units go into the core meanwhile (az_boot reads AZ.INI; the OSD's
  // saved images override units 0..3 - the machine must not be running
  // on the empty memory while its ROMs arrive); then release it
...
static void menu_draw_title(menu_t *menu, const char *s) {
  ...
  if(!menu->form && core_id == CORE_ID_BK && CORE_VERSION[0]) {
...
static void menu_debug_open(menu_t *menu, const char *title) {
  // top.v's dbg bus through SYS command 7: what the machine is doing
  static unsigned char d[32];
  static char line[10][40];
  static const char *paras[20];
  int n = 0;
  sys_get_debug(menu->osd->spi, d, 32);
  ...
  for(int i=0;az_boot_line(i) && n < 19;i++) paras[n++] = az_boot_line(i);   // the ROM load, its read-back verify, the commands served
  paras[n] = NULL;
...
static void menu_fileselector(menu_t *menu, int event) {
  ...
  if(event == FSEL_INIT) {
	...
	if(entry->name[0] == '/') {
	  ...
	  menu_goto_form(menu, parent, fsel_entry);
	  if(core_id == CORE_ID_BK) menu_bk_mount(drive, NULL);
	...
	// an AZ unit: remembered for the settings, mounted through azbk.c
...
static void menu_select(menu_t *menu) {
  ...
  case 'T':
    ...
    if(core_id == CORE_ID_BK) {
      ...
      else                             menu_text_open(menu, label, about_bk);
    }
    break;
