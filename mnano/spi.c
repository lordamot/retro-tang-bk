...
#warning "BITBANG SPI"
  ...
  // BK Nano: 2048 words, not the siblings' 512.  This task serves the AZ
  // controller's commands (azbk.c), which run FatFs - built with the
  // long-name buffer on the stack (FF_USE_LFN 2: 512 bytes) - and the
  // card layer's printfs from here; the siblings never called FatFs
  // from this task.  With 512 words the fourth board (23 Sep 2026) hung
  // in the first block read with the OSD dead: the stack-overflow hook.
