...
// BK Nano: the two waits below are bounded.  Unbounded, a card that
// does not answer spins here for ever with the SPI mutex held, and the
// OSD dies with it; bounded, the read fails, FatFs reports it, the
// controller answers ERR, and the Debug page shows the count.
...
// the image slots as the ZS-256 core uses them (sd_card.v's
// image_mounted index): 0..3 floppies A..D, 4 the SMUC's disk; 5 the
// extra slot, browsed only
static const char *drivename(int drive) {
  static const char *names[] = { "A", "B", "C", "D", "HDD", "ROM" };
  return names[drive];  
}
