...
module hdmi_tx (
    input             I_rst_n      ,
    input             I_rgb_clk    ,   // pixel clock, 64.8 MHz
    input             I_rgb_vs     ,
    input             I_rgb_hs     ,
    input             I_rgb_de     ,
    input       [7:0] I_rgb_r      ,
...
//
// N = 6144 is the standard value for the 48 kHz family; CTS = 64800 puts
// Fs at 64.8e6 * 6144 / (128 * 64800) = 48.000 kHz exactly - the 64.8 MHz
// pixel clock of this design - and the 48 kHz in the channel status is
// true.  (ZS-256 Nano's 42 MHz took CTS 42000.)  (Korvet Nano's 40.5 MHz took CTS 40500, PK8000 Nano's 30 MHz
// 30000; UKNC Nano's 50.14 MHz needed 50140, and the text above is its
// story.)
//------------------------------------------------------------------------
localparam [19:0] ACR_N   = 20'd6144 ;
localparam [19:0] ACR_CTS = 20'd64800;

localparam integer AUD_PERIOD = 128 * 64800;   // 8294400 accumulator wrap
...
localparam integer DI_PKTS  = 3                     ; // packets in one island (160-clock back porch here)
