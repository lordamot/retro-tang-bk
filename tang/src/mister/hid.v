...
// output HID data received from USB
output [5:0]     mouse;
// two bytes a key event, as usb_host.c's kbd_tx_bk sends them: the
// БК's КОИ-7 code, then the flags (keyboard.v says which), with a strobe
// on the second.  Two equal events in a row are two events.
output reg [7:0] keyboard;
output reg [7:0] keyboard_flags;
   ...
   if(reset) begin
      ...
      if(data_in_strobe) begin      
        if(data_in_start) begin
            ...
            // CMD 1: keyboard data - the code, then the flags
            if(command == 8'd1) begin
                if(state == 4'd1) keyboard <= data_in;
                if(state == 4'd2) begin keyboard_flags <= data_in; keyboard_stb <= 1'b1; end
            end
